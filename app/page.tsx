...
function PhaseDots({ draftDone, codeDone }: { draftDone: boolean; codeDone: boolean }) {
	return (
		<div
			className="flex items-center gap-2"
			title={`Draft ${draftDone ? '✓' : '…'} · Code ${codeDone ? '✓' : '…'}`}
		>
			<span
				aria-label={`Draft ${draftDone ? 'complete' : 'incomplete'}`}
				className={
					draftDone
						? 'w-2.5 h-2.5 rounded-full bg-amber border border-amber'
						: 'w-2.5 h-2.5 rounded-full border border-dashed border-amber/70'
				}
			/>
			<span
				aria-label={`Code ${codeDone ? 'complete' : 'incomplete'}`}
				className={
					codeDone
						? 'w-2.5 h-2.5 rounded-full bg-sage border border-sage'
						: 'w-2.5 h-2.5 rounded-full border border-dashed border-sage/70'
				}
			/>
		</div>
	)
...
export default async function Home({
	...
	const problems = await listProblemsWithProgress()
								...
								const content = (
									<div className="grid grid-cols-[44px_1fr_auto] items-baseline gap-6 py-6 border-b border-rule-soft group hover:bg-paper/50 transition-colors px-1 -mx-1">
										...
										<div className="flex items-center gap-5 shrink-0">
											{canOpen && (
												<PhaseDots draftDone={p.draft_done} codeDone={p.code_done} />
											)}
